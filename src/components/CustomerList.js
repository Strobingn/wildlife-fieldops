...
export const CustomerList = {
  ...
  afterRender(state) {
    ...
    if (loadMore) {
      const handler = () => { this._page++; state.rerender?.(); };
  ...
  _renderCustomerDetail(customer, allJobs, state) {
    const customerJobs = allJobs.filter(
      (j) => j.customer_id === customer.id || j.customer === customer.name
    );
    ...
    return /* html */ `
      ...
      <div id="customerJobs">
        ${customerJobs.length
          ? customerJobs.sort((a, b) => new Date(b.created_at || b.created || 0) - new Date(a.created_at || a.created || 0)).map((j) => {
              const icon = { "Raccoon": "🦝", "Grey Squirrel": "🐿️", "Red Squirrel": "🐿️", "Flying Squirrel": "🦇", "Bat": "🦇", "Skunk": "🦨", "Groundhog": "🦫", "Bird": "🐦", "Snake": "🐍", "Opossum": "🦡", "Rodent": "🐁", "Mouse": "🐁", "Rat": "🐀", "Carpenter Bee": "🐝" }[j.species] || "🐾";
              const sc = { "Active": "active", "Scheduled": "scheduled", "Closed": "closed", "Trapping": "trapping", "Repair": "repair", "Waiting On Customer": "scheduled", "Exclusion": "active", "Warranty": "active", "Cancelled": "closed" }[j.status] || "active";
              return `
