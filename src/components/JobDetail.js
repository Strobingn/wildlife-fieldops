...
export const JobDetail = {
  ...
  render(state) {
    ...
    const timerTotal = (job.timer_total || 0) + (timerRunning ? Math.ceil((Date.now() - new Date(job.timer_start).getTime()) / 60000) : 0);
  ...
  afterRender(state) {
    ...
    if (job?.timer_start) {
      this._timerInterval = setInterval(() => {
        ...
        if (display) {
          const elapsed = (job.timer_total || 0) + Math.ceil((Date.now() - new Date(job.timer_start).getTime()) / 60000);
  ...
  _renderPropertyHistory(job, allJobs) {
    const related = allJobs.filter(
      (j) =>
        j.id !== job.id &&
        (j.address?.toLowerCase() === job.address?.toLowerCase() || j.phone === job.phone)
    );
