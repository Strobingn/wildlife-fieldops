...
export const GPSTracker = {
  ...
  render(state) {
    ...
    return /* html */ `
      ...
      ${this._mapView === "heatmap"
          ...
          ? sortedTowns.map(([town, data]) => `
            <div class="card">
              ...
              <div style="margin-top:6px;">
                ${Object.entries(data.species).sort((a, b) => b[1] - a[1]).map(([sp, n]) => `<span class="pill">${SPECIES_ICONS[sp] || "🐾"} ${E(sp)} ${n}</span>`).join("")}
              </div>
        ...
        ${jobs.length
          ? jobs.slice().sort((a, b) => new Date(b.created_at || b.created || 0) - new Date(a.created_at || a.created || 0)).map((j) => `
  ...
  afterRender(state) {
    ...
    document.querySelectorAll("[data-action='view-markers']").forEach((btn) => {
      const handler = () => { this._mapView = "markers"; state.rerender?.(); };
      btn.addEventListener("click", handler);
      this._listeners.push({ el: btn, type: "click", fn: handler });
    });
    document.querySelectorAll("[data-action='view-heatmap']").forEach((btn) => {
      const handler = () => { this._mapView = "heatmap"; state.rerender?.(); };
      btn.addEventListener("click", handler);
      this._listeners.push({ el: btn, type: "click", fn: handler });
    });
  ...
  _initMap(state) {
    ...
    const jobs = (state.jobs || []).filter((j) => j.latitude && j.longitude);
    if (!jobs.length) {
      container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--muted);">No GPS jobs to display</div>';
      return;
    }
