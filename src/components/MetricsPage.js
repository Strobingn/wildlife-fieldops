...
export const MetricsPage = {
  ...
  render(state) {
    ...
    const speciesCounts = {};
    jobs.forEach((j) => { speciesCounts[j.species || "Other"] = (speciesCounts[j.species || "Other"] || 0) + 1; });
    const maxSpeciesCount = Math.max(...Object.values(speciesCounts));

    // ─── Status distribution ───
    const statusCounts = {};
    jobs.forEach((j) => { statusCounts[j.status || "Unknown"] = (statusCounts[j.status || "Unknown"] || 0) + 1; });
    ...
    // ─── Jobs by town ───
    const townCounts = {};
    jobs.forEach((j) => { townCounts[j.town || "Unsorted"] = (townCounts[j.town || "Unsorted"] || 0) + 1; });
    ...
    const sortedMonths = Object.entries(monthCounts).sort((a, b) => new Date(a[0]) - new Date(b[0])).slice(-12);
    ...
    const avgResponse = responseTimes.length ? (responseTimes.reduce((a, b) => a + b, 0) / responseTimes.length).toFixed(1) : "N/A";
    ...
    return /* html */ `
      ...
      ${Object.keys(techRevenue).length > 1 || Object.keys(techRevenue)[0] !== "Unassigned"
        ? `<div class="section-title">Revenue by Technician</div>
           <div class="card">
             ${Object.entries(techRevenue).sort((a, b) => b[1] - a[1]).map(([tech, rev]) => {
      ...
      <div class="card">
        ${Object.entries(townCounts).sort((a, b) => b[1] - a[1]).slice(0, 15).map(([town, count]) => {
