...
export const SignaturePad = {
  ...
  render(state) {
    ...
    const signatures = (state.photos || []).filter((p) => p.tag === "Customer signature" && (jobId ? p.job_id === jobId : true));
      ...
      ${!jobId
        ? `<div class="card">
            ...
            <select id="sigJobSelect">
              ...
              ${jobs.filter((j) => j.status !== "Closed" && j.status !== "Cancelled").map((j) => `<option value="${j.id}">${E(j.title || j.species + " job")} — ${E(j.customer)}</option>`).join("")}
            </select>
           </div>`
        : `<div class="card">
  ...
  afterRender(state) {
    ...
    if (nameEl) {
      const handler = () => { this._signerName = nameEl.value; };
